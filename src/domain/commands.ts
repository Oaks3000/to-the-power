...
export type GameCommand =
  | { type: "set_school_year"; schoolYear: SchoolYear }
  | { type: "change_role"; role: CareerLevel }
  | { type: "change_tempo"; tempo: TempoState }
  | { type: "advance_time"; hours?: number }
  | { type: "submit_challenge_answer"; topic: MathsTopic; correct: boolean; mode?: ChallengeMode }
  | { type: "start_timed_challenge"; challengeId: string; timerHours?: number }
  | { type: "register_latent_consequence"; consequence: LatentConsequence }
  | { type: "change_dark_index"; delta: number }
  | { type: "change_npc_relationship"; npcId: string; delta: number };
...
export function decideWithResult(state: GameState, command: GameCommand): CommandDecision {
  ...
  switch (command.type) {
    ...
    case "submit_challenge_answer":
        ...
        command.mode === undefined || command.mode === "decision" || command.mode === "gate" || command.mode === "crisis",
        "submit_challenge_answer.mode is invalid"
      );
      return {
        events: [{
...
export function decide(state: GameState, command: GameCommand): GameEvent[] {
  return decideWithResult(state, command).events;
}

export function executeCommandWithResult(state: GameState, command: GameCommand, rng: Rng = Math.random): CommandResult {
...
export function executeCommand(state: GameState, command: GameCommand, rng: Rng = Math.random): GameState {
