...
import type { ActiveTimedChallenge, CurriculumBand, GameEvent, GameState, MathsTopic, Rng } from "./types.js";

export interface ReduceOptions {
  evaluateLatent?: boolean;
  generateFollowUps?: boolean;
}

function logEvent(state: GameState, event: GameEvent): GameState {
  return {
    ...state,
...
function applyCoreEvent(state: GameState, event: GameEvent): { state: GameState; followUps: GameEvent[] } {
  switch (event.type) {
    case "SchoolYearSet": {
    ...
    case "TimeAdvanced": {
      ...
      const survivors: Record<string, ActiveTimedChallenge> = {};
    ...
    case "ChallengeAttempted": {
      ...
      const typedTopic = topic as MathsTopic;
    ...
    case "RemediationTriggered": {
      ...
      const typedTopic = topic as MathsTopic;
      const typedBand = band as CurriculumBand;
      return {
        state: {
          ...state,
          pendingRemediations: [
    ...
    case "LatentConsequenceRegistered": {
      ...
      return {
        state: {
          ...state,
          pendingLatentConsequences: [...state.pendingLatentConsequences, consequence as GameState["pendingLatentConsequences"][number]]
        },
        followUps: []
      };
    }
