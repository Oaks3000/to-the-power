...
function mergeAverageByKey(buckets) {
    const output = {};
...
function computeWilsonInterval(numerator, denominator, z = Z_95) {
    if (denominator <= 0) {
        return {
            lower: 0,
            estimate: 0,
...
function computeMeanInterval(values, z = Z_95) {
    if (values.length === 0) {
        return {
            lower: 0,
            estimate: 0,
...
function computeConfidence(runs, aggregates) {
...
function evaluateCohortGates(aggregates, confidence, targets) {
...
function isSignificantRateExceedance(rate, target, attempts) {
    if (attempts <= 0) {
        return false;
    }
...
export function detectScenarioOutliers(runs, inputTargets = {}) {
    if (runs.length < 2) {
        return [];
    }
    ...
    return runs.flatMap((run) => {
        const reasons = [];
        if ((remediationStats.stdDev > 0 && run.metrics.remediationCount > remediationStats.mean + (1.5 * remediationStats.stdDev))
            || run.metrics.remediationCount > targets.maxRemediationsPerRun) {
            reasons.push(`remediations unusually high (${run.metrics.remediationCount}; target <= ${targets.maxRemediationsPerRun}; avg ${remediationStats.mean.toFixed(2)})`);
        }
        if ((crisisRateStats.stdDev > 0 && run.metrics.crisisFailureRate > crisisRateStats.mean + (1.5 * crisisRateStats.stdDev))
            || isSignificantRateExceedance(run.metrics.crisisFailureRate, targets.maxCrisisFailureRate, run.metrics.totalCrisisAttempts)) {
            reasons.push(`crisis failure rate unusually high (${(run.metrics.crisisFailureRate * 100).toFixed(1)}%; target <= ${(targets.maxCrisisFailureRate * 100).toFixed(1)}%; avg ${(crisisRateStats.mean * 100).toFixed(1)}%)`);
        }
        const juniorPromotionHour = run.metrics.roleTransitionHours.junior_minister;
        if ((juniorStats.stdDev > 0 && typeof juniorPromotionHour === "number" && juniorPromotionHour > juniorStats.mean + (1.5 * juniorStats.stdDev))
            || (typeof juniorPromotionHour === "number" && juniorPromotionHour > targets.maxJuniorMinisterTransitionHour)) {
            reasons.push(`junior_minister promotion unusually late (${juniorPromotionHour}h; target <= ${targets.maxJuniorMinisterTransitionHour}h; avg ${juniorStats.mean.toFixed(1)}h)`);
        }
        return reasons.length === 0 ? [] : [{ seed: run.seed, reasons }];
    });
}
export function buildScenarioTelemetryReport(scenario, runs, targets = {}) {
...
export async function runScenarioSweep(scenario, options) {
    const runs = [];
    for (const seed of options.seeds) {
        const runOptions = { seed };
