function isActive(consequence, atHour) {
    if (consequence.expiresAtHour === undefined) {
        return true;
    }
    return consequence.expiresAtHour >= atHour;
}
export function evaluateActivationCondition(state, consequence) {
...
export function evaluateLatentConsequences(state, rng = Math.random) {
    const survivors = [];
    const triggered = [];
