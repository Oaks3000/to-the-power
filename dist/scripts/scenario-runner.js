...
function parseOptionalNumberArg(args, key) {
...
async function run() {
    ...
    if (args.seeds && args.seeds.length > 0) {
        ...
        console.log(`Crisis failure rate CI95: ${(report.confidence.crisisFailureRate.lower * 100).toFixed(1)}%-${(report.confidence.crisisFailureRate.upper * 100).toFixed(1)}%`);
        console.log(`Remediations/run CI95: ${report.confidence.remediationsPerRun.lower.toFixed(2)}-${report.confidence.remediationsPerRun.upper.toFixed(2)}`);
        console.log(`Junior transition hour CI95: ${report.confidence.juniorMinisterTransitionHour.lower.toFixed(1)}-${report.confidence.juniorMinisterTransitionHour.upper.toFixed(1)}`);
